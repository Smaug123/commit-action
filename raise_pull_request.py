...
if not PR_TITLE:
    raise Exception("Supply PR_TITLE env var")
...
BRANCH_PREFIX = os.environ.get("BRANCH_PREFIX") or "auto_pr"
...
def main():
    ...
    branch_name = BRANCH_PREFIX + datetime.datetime.fromtimestamp(time.time()).strftime('%Y_%m_%d-%H_%M_%S_%f')
    create_branch(branch_name, new_commit_sha)
    print(f"Branch created: {branch_name}")

    url, pr_num = create_pull_request(title=PR_TITLE, branch_name=branch_name, base_branch=DEFAULT_BRANCH)
